...
export const getSocketIO = (server?: any): Server => {
  if (io) return io
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('leave_conversation', (data) => {
      ...
      activeConversations.get(conversation_id)?.delete(data.user_id)
    ...
    socket.on('disconnect', () => {
      ...
      activeConversations.forEach((users) => {
        users.forEach((userId) => {
          io?.emit('user_disconnected', { user_id: userId })
        })
      })
    })
  })
